 ...
 * @param {Worker} worker
 * @param {function} callback A callback to be fired when comment changes are deteced
 * @returns {Guardian}
 */
var Guardian = function(worker, callback) {
	// Initiate the members
	var host = worker.url.split('/')[2];
	...
	// The removeGuardian needs this reference as worker.tab becomes null on detach
	var tab = worker.tab;
...
Guardian.prototype.onComments = function(comments) {
	...
	if (oldCount !== comments)
		Guardian.onComments(this.worker, this.callback);
};
